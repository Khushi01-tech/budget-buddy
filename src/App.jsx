...
export default function App() {
  ...
  const s = {
    ...
    sidebar: { width: '220px', background: '#13151e', display: 'flex', flexDirection: 'column', padding: '0', position: 'fixed', height: '100vh', borderRight: '1px solid #1e2130' },
    ...
    main: { marginLeft: '220px', flex: 1, padding: '32px', background: '#0d1117', minHeight: '100vh' },
    pageTitle: { fontSize: '22px', fontWeight: '700', color: '#f1f5f9', marginBottom: '6px' },
    pageSubtitle: { fontSize: '13px', color: '#6b7280', marginBottom: '28px' },
    ...
    card: { background: '#13151e', borderRadius: '14px', padding: '22px', border: '1px solid #1e2130' },
    ...
    <div style={s.app}>
      ...
      <div style={s.sidebar}>
        ...
        <div style={s.balanceCard}>
          ...
          <div style={{ fontSize: '11px', color: summary.balance >= 0 ? '#10b981' : '#ef4444', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
        ...
        <div style={s.navSection}>
          ...
          {navItems.map(item => (
            <div key={item.id} onClick={() => setActivePage(item.id)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', cursor: 'pointer', marginBottom: '2px', background: activePage === item.id ? '#1e2130' : 'transparent', color: activePage === item.id ? '#f1f5f9' : '#6b7280', fontWeight: activePage === item.id ? '600' : '400', fontSize: '13px', transition: 'all 0.15s' }}>
              <Icon type={item.icon} />
              {item.label}
        ...
        {activePage === 'budgets' && (
          <div>
            ...
            <div style={{ ...s.card, marginBottom: '20px' }}>
              ...
              <form onSubmit={handleBudget} style={{ display: 'flex', gap: '10px' }}>
                ...
                <button type="submit" style={s.btnPrimary}>Set Limit</button>
              </form>
            </div>
                ...
                return (
                  <div key={i} style={s.card}>
                    ...
                    <div style={{ background: '#1e2130', borderRadius: '99px', height: '5px', marginBottom: '8px' }}>
                      <div style={{ background: over ? '#ef4444' : '#4f46e5', width: `${pct}%`, height: '5px', borderRadius: '99px', transition: 'width 0.3s' }} />
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280' }}>{pct.toFixed(0)}% used</div>
                      <button onClick={() => handleDeleteBudget(b.id)} style={{ ...s.btnDanger, padding: '4px 8px', fontSize: '11px', color: '#6b7280' }}>
                        <Icon type="delete" />
                      </button>
                    </div>
                  </div>
